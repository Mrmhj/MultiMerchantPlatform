...
```
下单 ──预占库存(reserve)──▶ 预占失败 → 释放已占 + 拒绝下单（400 STOCK_INSUFFICIENT）
支付 ──扣减库存(confirm)──▶ 预占转扣减（失败记录日志，不阻塞）
取消 ──释放库存(release)──▶ 预占回滚
```

- 跨服务调用 stock-service 内部接口（X-Internal-Key 校验）
- **多 SKU 部分失败补偿**：预占逐项执行，任一失败自动释放已预占项并拒绝下单（分布式一致性）
...
- **支付对接**：`/pay` 当前为模拟确认；正式版由 pay-service（已接入，见 v5.1）回调
- **价格信任**：当前单价由客户端提交（快照）；正式版从 product-service 拉取校验
- **库存联动**：✅ 已接线（下单预占/支付扣减/取消释放，见核心设计）
